...
class CubeRotateZAxisTouchObjects(leap_hand_base.LeapHandEnv):
  """Rotate a cube around the z-axis with touch sensor observations (objects)."""
...
def create_mask_class(
  ...
  """Create a CubeRotateZAxisTouchObjects subclass with a baked-in fixed mask."""
  ...
  def __init__(
    ...
    CubeRotateZAxisTouchObjects.__init__(
        self,
        config=config,
        config_overrides=config_overrides,
  ...
  cls = type(class_name, (CubeRotateZAxisTouchObjects,), {
      "__init__": __init__,
...
def _create_touch_mask_classes(mask_path: str):
  """Load masks from JSON and create CubeRotateZAxisTouchObjectsMask1..N classes,
  plus noise-level variants CubeRotateZAxisTouchObjectsMask{N}Noise{PCT}."""
  ...
  for idx, mask in enumerate(all_masks):
    ...
    class_name = f"CubeRotateZAxisTouchObjectsMask{mask_n}"
    ...
    for pct in TOUCH_SENSOR_NOISE_PCT_LEVELS:
      if pct == 0:
        continue
      noise_class_name = f"CubeRotateZAxisTouchObjectsMask{mask_n}Noise{pct}"
    ...
    finger_noise_class_name = f"CubeRotateZAxisTouchObjectsMask{mask_n}FingerNoise"
